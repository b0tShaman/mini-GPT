...
def train_model():
    ...
    if model:
        choice = input(
            "   Found saved model. (c)ontinue training, (n)ew training, or (s)kip to inference? [c/n/s]: "
        ).lower()
        if choice == "s":
    ...
    if processor is None:
        processor = TextProcessor(
            context_len=config.CONTEXT_LEN, vocab_size=config.MAX_VOCAB_SIZE
        )
    ...
    if model is None:
        print(
            f"🏗️  Building new model: Layers={num_layers}, Heads={num_heads}, Embed={embed_dim}, Context={config.CONTEXT_LEN}"
        )
        model = TextGenModel(
    ...
    for epoch in range(config.EPOCHS):
        ...
        with torch.no_grad():
            for x_val, y_val in val_loader:
                ...
                v_loss = criterion(
                    val_logits.view(B_v * T_v, V_v), y_val.view(B_v * T_v)
                )
